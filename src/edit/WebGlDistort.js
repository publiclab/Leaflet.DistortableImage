...
function warpWebGl(id, matrix1, matrix2, download) {
  ...
  try {
	  var canvas = window.canvas.fx.canvas(1500,1500);
  } catch (e) {
	  Window.alert(e);
	  return;
  }
  ...
  image.onload = function ()
  {
	  ...
	  var texture = window.canvas.texture(image);
	  ...
	  for (i = 0; i < matrix2.length; i += 2)
	  {
		  matrix2Xs.push(matrix2[i]);
	  }

	  for (i = 1; i < matrix2.length; i += 2)
	  {
	  ...
	  window.canvas.draw(texture,
		  image.width,//  * ratio,
		  image.height// * ratio
	  );

	  var ratioY = (matrix2southmost - matrix2northmost) / (matrix1southmost - matrix1northmost);

	  var ratioX = (matrix2eastmost - matrix2westmost) / (matrix1eastmost - matrix1westmost);
	  ...
	  window.canvas.perspective(matrix1, matrix2).update();

	  // replace the image with the canvas
	  // image.parentNode.insertBefore(canvas, image);
	  // image.parentNode.removeChild(image);

	  var burl = canvasToBlobUrl(window.canvas);
