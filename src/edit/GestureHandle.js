L.GestureHandle = L.RotateScaleHandle.extend({
  options: {
    TYPE: "gesture",
    icon: new L.Icon({
      iconUrl:
  ...
  _onHandleDrag: function() {
    ...
    if (this._handled.hasOwnProperty("edgeMinWidth")) {
      ...
      if ((w > edgeMinWidth && h > edgeMinWidth) || scale > 1) {
        ...
        var oldT = this._icon.style.transform;
        window.oldT = oldT;
        this._icon.style.transform = oldT + "scale(" + scale + ")";
      }
    }
  ...
  updateHandle: function() {
    ...
    // this._icon.width = this._handled._image.width;
    // this._icon.height = this._handled._image.height;
    // this.iconSize
    window.thisa = this;
  }
});
