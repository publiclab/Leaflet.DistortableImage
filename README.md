...
The "reveal" tool extends the native `EditOverlayAction` Leaflet class behaviour and hides some user-specified tools, hence saving up space in the toolbar and giving more room to your mapview! Reference the comments below for a detailed explaination on what happens when the "reveal" tool is executed. For custom toolbar instantiation reference, please refer to the section (`Adding custom toolbars`) below.
...
```js
var keymapper = {
  position: "topright", // "top" and "bottom" options can be combined with "left" and "right" as per user's discretion
  el: "div",
